...
export default function Item(props) {
  const { containerClassName, item, editItem, index } = props;
  const [flipCard, setFlipCard] = useState(false);
  const [activeRow, setActiveRow] = useState(false);
  const [tooltipEdit, setTooltipEdit ] = useState(false);
  const { title, isbn, totalNumber, itemType, itemCategory,description, itemCode, author, dateAdded } = item;
          ...
          <div className="item-card-back w-full h-full">
            <div className="item-card-back-inner">
              <div className="item-card-info">
                <span className="p-4 text-gray-800">{totalNumber}</span>
              ...
              <div className="item-card-back-edit" onClick={(e) => editItem(index)} onMouseEnter={(e) => setTooltipEdit(true)} onMouseLeave={(e) => setTooltipEdit(false)} style={{ backgroundImage: `url(/${images.edit_icon})` }}><span className={tooltipEdit ? `${tooltipClassName} show-tooltip` : tooltipClassName}>edit</span></div>
