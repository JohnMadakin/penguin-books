  ...
  */
  ...
  async function submitItems(e){
    ...
    if (!formErrorsOccured){
      ...
      const results = await ItemService.postItem(formData, token, url);
      if (results.status == 'success') {
  ...
  return (
    <div>
{/*------------------header-----------------start*/}
      <div className="modal-close absolute top-0 right-0 cursor-pointer flex flex-col items-center mt-4 mr-4 text-white text-sm z-50" onClick={toggleModal}>
