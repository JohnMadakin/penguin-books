...
export default function ViewUsersBorrowedItems(props) {
  ...
  useEffect(() => {
    ...
    const url = `${base_url}/api/v1/users/${userId}/items`;
    setUsersName(location.user)

    async function loadUserItems() {

      const result = await itemService.getUserItems(token, url);
      ...
      if (result.status == 'error') {
        ...
        if (!errorMessage) {
          toastNotify('info', 'info', 'User is not owing the library', 'topRight');
    ...
    <div className="mx-auto">
      <div className="flex flex-wrap flex-row justify-between mx-3 mb-6 p-5  w-11/12 border-solid border-b border-gray-400 text-2xl"> 
        <div className="flex flex-wrap flex-row w-64">
          ...
          <span style={{ fontSize: '1.5rem' }}>{usersName}</span>
        </div>
